  ...
  function startRecursive() {
    ...
    function sumTenIds() {
      for (let i = 9; i >= 0; i--) {                                  // Перебераю 10 элементов массива пользователей
        const deleteUser = users.splice(i, 1);                        // Беру пользователя для промиса, сразу удаляя его из начального массива
        arr[i] = processUser(deleteElemnt[0].id).then(userId => {     // Отдаю его функции с промисом и возвращаю его ID
          return(userId);
        });
      }
      totalArr = totalArr.concat(sumPromiseResult(arr));              // Суммирую результаты обработки 10 пользователей и занашу их в массив
      if (users.length !== 0) {                                       // Проверяю наличие пользователей в изначальном массиве
        sumTenIds();                                                  // Если они есть, то снова запускаю функцию для их обработки
      }
      return sumPromiseResult(totalArr);                              // Иначе возвращаю сумму ID всех пользователей
    }
